...
while (nb_good_try < total_EM_tries)
    ...
    model = init_regression_param_MRHLP(XBeta, y, K, type_variance, try_EM);
    ...
    Winit = W0;%
    %% EM %%%%
    while ~converge && ~isempty(model) && (iter< max_iter_EM)
        %% E-Step
        piik = logit_model(Winit,Xw);
        
        log_piik_fik =zeros(m,K);
        for k = 1:K
            muk = XBeta*model.betak{k};
            if homoskedastic
                sigmak =  model.sigmak;
            else
                sigmak = model.sigmak{k};
            end
            z =((y-muk)*inv(sigmak)).*(y-muk);
            mahalanobis = sum(z,2);
            denom = (2*pi)^(d/2)*(det(sigmak))^(1/2);
            
            log_piik_fik(:,k) = log(piik(:,k))- ones(m,1)*log(denom)- 0.5*mahalanobis;
        end
        ...
        %
        for k=1:K
            weights = tauik(:,k);% post prob of each component k (dimension nx1)
        ...
        res = IRLS(Xw, tauik, Winit, verbose_IRLS);
        model.wk = res.W;
        Winit = res.W;
        stats.piik = res.piik;
    ...
    mrhlp.stats.piik = stats.piik(1:m,:);
    mrhlp.stats.tauik = tauik(1:m,:);
    
    mrhlp.stats.log_piik_fik = log_piik_fik;
    
    %% estimated parameter vector
    %theta = [model.wk(:); model.betak(:); model.sigmak(:)];
...
% % for the best mrhlp
mrhlp.stats.piik = mrhlp.stats.piik;
mrhlp.stats.tauik = mrhlp.stats.tauik;

%% classsification pour EM : classes = argmax(piik) (here to ensure a convex segmentation of the curve(s)).
[klas, Zik] = MAP(mrhlp.stats.piik);
...
for k = 1:K
    regimesk =  XBeta*mrhlp.model.betak{k};
    mrhlp.stats.polynomials(:,:,k) = regimesk;
    mrhlp.stats.weighted_polynomials(:,:,k) = (mrhlp.stats.piik(:,k)*ones(1,d)).*regimesk;
end
...
%
mrhlp.stats.nu = nu;
mrhlp.stats.design.XBeta = XBeta;
mrhlp.stats.design.Xw = Xw;
