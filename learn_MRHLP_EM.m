...
while (nb_good_try < total_EM_tries)
    ...
    mrhlp.loglik = loglik;
    mrhlp.stored_loglik = stored_loglik;
    mrhlp.param = param;
    mrhlp.log_piik_fik = log_piik_fik;
    
    %% estimated parameter vector
    theta = [param.wk(:); param.betak(:); param.sigma(:)];
...
for k = 1:K
    ...
    mrhlp.polynomials(:,:,k) = regimesk;
    mrhlp.weighted_polynomials(:,:,k) = (mrhlp.param.piik(:,k)*ones(1,d)).*regimesk;
end
mrhlp.Ey =sum(mrhlp.weighted_polynomials,3);%sum over the regimes k

mrhlp.cputime = mean(cputime_total);
mrhlp.cputime_total = cputime_total;

% mrhlp.log_fxi = log_fxi(1:m,:);
% mrhlp.fxi = fxi(1:m,:);
...
mrhlp.BIC = mrhlp.loglik - (nu*log(m)/2);
mrhlp.AIC = mrhlp.loglik - nu;
%% CL(theta) : Completed-data loglikelihood
zik_log_piik_fk = Zik.*mrhlp.log_piik_fik;
sum_zik_log_fik = sum(zik_log_piik_fk,2);
comp_loglik = sum(sum_zik_log_fik);
mrhlp.comp_loglik = comp_loglik;
mrhlp.ICL = mrhlp.comp_loglik - (nu*log(m)/2);
%warning on
%
mrhlp.nu = nu;
mrhlp.XBeta = XBeta;
