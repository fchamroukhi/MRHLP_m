...
[~, K] = size(mrhlp.stats.piik);
for k=1:K
    model_k = mrhlp.stats.polynomials(:,:,k);
    
    active_model_k = model_k(mrhlp.stats.klas==k,:);%prob_model_k >= prob);
    active_period_model_k = x(mrhlp.stats.klas==k);%prob_model_k >= prob);
    
    inactive_model_k = model_k(mrhlp.stats.klas ~= k,:);%prob_model_k >= prob);
    inactive_period_model_k = x(mrhlp.stats.klas ~= k);%prob_model_k >= prob);
...
%subplot(212),
for k=1:K
    plot(x,mrhlp.stats.piik(:,k),'Color', colors{k},'linewidth',1.5);
    hold on
end
...
hold on, plot(x,mrhlp.stats.Ey,'r','linewidth',1.5);
xlim([x(1), x(end)])

% transition time points
tk = find(diff(mrhlp.stats.klas)~=0);
...
axes2 = axes('Parent',figr,'Position',[0.1 0.06 0.8 0.35],'FontSize',14);
box(axes2,'on'); hold(axes2,'all');
plot(x,mrhlp.stats.klas,'k.','linewidth',1.5);
xlim([x(1), x(end)])
...
% %
% figure,
% plot(mrhlp.stats.stored_loglik,'-','linewidth',1.5)
